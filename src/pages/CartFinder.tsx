...
//   /**
//    * -----------------------------------------------------------------
//    * The "Optimizer" Brain (with Logging)
//    * -----------------------------------------------------------------
//    */
//   const findBestCart = (
...
//     console.log("--- findBestCart: START ---");
...
//       });
//       const bestDeal = itemDeals[0];
//       candidateStoreIds.add(`${bestDeal.retailer}@${bestDeal.zip_code}`);
//     }

//     console.timeEnd("Optimizer: 1. Pre-processing");
//     console.log(`findBestCart: Pre-processing complete. Found ${candidateStoreIds.size} candidate stores.`);

//     if (candidateStoreIds.size > MAX_CANDIDATE_STORES) {
//       console.warn("findBestCart: Too many candidate stores. Aborting optimization.");
...
//     console.log(`findBestCart: Generated ${allCombos.length} valid combinations.`);
...
//         // --- ⬇️ MODIFIED: Bypassing the render ⬇️ ---

//         console.log("--- !!! FINAL OPTIMIZER RESULT (bestCart) !!! ---");
//         console.log(bestCart);
        
//         console.log("--- !!! FINAL 1-STORE RESULTS (allStoreCarts) !!! ---");
//         console.log(allStoreCarts);

//         // We will NOT call setResults or setSingleStoreResults
//         // Instead, we'll set a debug message.
//         setError("DEBUG: Check the console (F12) for optimizer results.");
...
export function CartFinder() {
  ...
  /**
   * -----------------------------------------------------------------
   * The "Optimizer" Brain (This is the topK=5 version)
   * -----------------------------------------------------------------
   */
  const findBestCart = (
    dealMenu: DealMenuItem[], 
    ...
    for (const item of searchTerms) {
      ...
      // Add all their unique stores to the candidate set
  ...
  /**
   * -----------------------------------------------------------------
   * Core Optimizer function (DEBUG Version - Bypasses Render)
   * -----------------------------------------------------------------
   */
  const handleRunOptimizer = async (searchTerms: string[]) => {
    console.log("--- handleRunOptimizer: START ---");
    ...
    try {
      ...
      if (dealMenu && dealMenu.length > 0) {
        ...
        // --- ⬇️ DEBUG: Bypassing the render ⬇️ ---
        ...
        // We will NOT call setResults or setSingleStoreResults
        // Instead, we'll set a debug message.
        setError("DEBUG: Check the console (F12) for optimizer results. Logic finished.");
    ...
    <div className="container mx-auto p-4">
      ...
      {initialSearchDone && (
        <div className="mt-6">
          ...
          <div className="mt-6">
            {/* --- Results are now hidden to prevent crash --- */}
            
            {/* RENDER LOGIC IS COMMENTED OUT FOR DEBUGGING */}
